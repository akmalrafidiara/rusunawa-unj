        ...
        <x-frontend.complaint.layout>
            <div class="p-8 text-center">
                ...
                <div class="mb-8">
                    ...
                    <div class="relative inline-flex items-center bg-gray-100 dark:bg-zinc-700 rounded-lg px-4 py-2 font-mono text-xl text-gray-800 dark:text-gray-200">
                        <span>{{ $report->unique_id }}</span>
                        {{-- Tombol untuk menyalin ID --}}
                        <button x-data="{ copied: false }" @click="navigator.clipboard.writeText('{{ $report->unique_id }}'); copied = true; setTimeout(() => copied = false, 2000);"
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-2">Gunakan ID ini untuk melacak status pengaduan Anda.</p>
                </div>

                <div class="flex flex-col sm:flex-row justify-center gap-4">
                    {{-- Tombol Lacak Pengaduan --}}
                    <a href="{{ route('complaint.track-complaint', ['unique_id' => $report->unique_id]) }}" class="px-6 py-3 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors duration-200" wire:navigate>
                        Lacak Pengaduan
                    </a>
                    {{-- Tombol Kembali ke Dashboard (asumsi ada rute dashboard) --}}
                    <a href="{{ route('occupant.dashboard') }}" class="px-6 py-3 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 transition-colors duration-200 dark:bg-zinc-700 dark:text-white dark:hover:bg-zinc-600" wire:navigate>
                        Kembali ke Dashboard
                    </a>
                    {{-- Tombol Buat Laporan Baru --}}
                    <a href="{{ route('complaint.create-complaint') }}" class="px-6 py-3 bg-green-600 text-white rounded-md hover:bg-green-700 transition-colors duration-200" wire:navigate>
                        Buat Laporan Baru
                    </a>
                </div>
            </div>
