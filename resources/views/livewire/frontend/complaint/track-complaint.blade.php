...
new #[Layout('components.layouts.frontend'), Title('Rusunawa UNJ | Lacak Pengaduan')] class extends Component
{
    ...
    protected $messages = [
        'reportIdToTrack.required' => 'ID Laporan wajib diisi.',
        'reportIdToTrack.string' => 'ID Laporan harus berupa teks.',
    ];

    protected function rules(): array
    {
        return [
            'reportIdToTrack' => 'required|string',
        ];
    }

    public function mount(?string $unique_id = null): void
    {
        ...
        if ($unique_id) {
            $this->reportIdToTrack = $unique_id;
            $this->fetchReportDetails($unique_id);
        } else {
            $this->reportIdToTrack = '';
        }
    }

    public function updatedReportIdToTrack(): void
    {
        $baseUrl = route('complaint.track-complaint', [], false);

        if (empty($this->reportIdToTrack)) {
            $this->js("history.pushState({}, '', '{$baseUrl}')");
            $this->showDetail = false;
            $this->reportNotFound = false;
            $this->foundReport = null;
            $this->resetErrorBag('reportIdToTrack');
    ...
    private function fetchReportDetails(?string $unique_id): void
    {
        if (empty($unique_id)) {
            $this->reportNotFound = false;
            $this->foundReport = null;
        ...
        // Eager load necessary relationships for detailed view
        $this->foundReport = Report::with(['reporter', 'contract.unit', 'currentHandler', 'logs.user', 'attachments'])
        ...
        if ($this->foundReport) {
            ...
            if ($this->canConfirm && $this->foundReport->completion_deadline) {
                $now = Carbon::now();
                $deadline = Carbon::parse($this->foundReport->completion_deadline);
                $this->confirmationDeadlineDaysLeft = $now->diffInDays($deadline, false);

                if ($now->isAfter($deadline) && !$this->isConfirmed) {
                    $this->foundReport->status = ReportStatus::CONFIRMED_COMPLETED;
                    $this->foundReport->save();
                    $this->isConfirmed = true;
                    $this->canConfirm = false;
                    $this->confirmationDeadlineDaysLeft = 0;
                    $this->foundReport->logs()->create([
                        'user_id' => null,
    ...
    public function submitForm(): void
    {
        ...
        $newUrl = route('complaint.track-complaint', ['unique_id' => $this->reportIdToTrack], false);
        $this->js("history.pushState({}, '', '{$newUrl}')");
    ...
    {
        if ($this->foundReport && $this->foundReport->status === ReportStatus::COMPLETED) {
            $this->foundReport->status = ReportStatus::CONFIRMED_COMPLETED;
            $this->foundReport->completion_deadline = null;
            $this->foundReport->save();
    ...
    // This method is specific to the second component, but we can include it if needed
