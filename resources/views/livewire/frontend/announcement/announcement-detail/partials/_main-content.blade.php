    ...
    @if ($announcement->image)
        <div class="overflow-hidden rounded-lg mb-6">
            <img src="{{ Storage::url($announcement->image) }}" alt="{{ $announcement->title }}"
                class="w-full h-max-96 object-cover object-center">
        </div>
    @endif
                    ...
                    <div class="w-full overflow-hidden">
                        ...
                        <div class="p-3 text-sm text-gray-700 dark:text-gray-300 font-medium break-words">
                            <flux:icon name="photo"
                                class="h-5 w-5 text-gray-400 dark:text-gray-500 inline-block mr-1" />
                            {{ $attachment->name }}
                        </div>
                    </div>
                @else
                    ...
                    <div>
                        @if (Str::contains($attachment->mime_type, 'pdf'))
                            ...
                            <div class="mb-4"> {{-- Margin bawah untuk memisahkan dari info file --}}
                                <div class="relative" style="padding-top: 56.25%;">
                                    <iframe
                                        src="{{ Storage::url($attachment->path) }}#toolbar=0&navpanes=0&scrollbar=0"
                                        class="absolute inset-0 w-full h-full" frameborder="0">
                                        Browser Anda tidak mendukung preview PDF. Silakan <a
                                            href="{{ Storage::url($attachment->path) }}"
                                            download="{{ $attachment->name }}"
                                            class="text-blue-600 hover:underline dark:text-blue-400 dark:hover:underline">unduh
                                            file</a> untuk melihatnya.
                                    </iframe>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center mt-2 p-2"> {{-- Tanpa bg, rounded, shadow, border --}}
                                ...
                                <flux:icon name="document"
                                    class="h-5 w-5 text-blue-500 dark:text-blue-400 mr-1 flex-shrink-0" />
                                <p
                                    class="font-medium text-sm text-gray-800 dark:text-gray-200 break-words flex-1 min-w-0 mr-3">
                                    {{ $attachment->name }}</p>
                        ...
                        @else
                            ...
                            <div class="p-4 bg-gray-100 dark:bg-zinc-700 rounded-lg text-center mb-4">
                                <p class="text-sm text-gray-600 dark:text-gray-300">Preview tidak tersedia untuk jenis
                                    file ini.</p>
                            </div>
