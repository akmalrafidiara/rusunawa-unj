<div>
    <div class="flex flex-col gap-6 w-full mx-auto items-center my-6 mb-6">
        <div class="text-left w-full">
            {{-- Tombol Kembali: Menggunakan link jika rute sebelumnya diketahui, atau history.back() jika dinamis --}}
            <a href="{{-- route('nama.rute.sebelumnya') --}}#" onclick="history.back(); return false;"
                class="inline-flex items-center text-sm lg:text-m text-black hover:text-green-800 dark:text-white dark:hover:text-zinc-900 mb-6">
                <flux:icon name="chevron-left" class="w-4 h-4 mr-1 text-green-600 dark:text-green-400" />
                Kembali
            </a>

            <h1 class="text-2xl font-bold text-gray-900 mb-2 dark:text-gray-100">Masuk ke Dashboard Kamar</h1>
            <p class="text-gray-600 text-sm lg:text-m dark:text-gray-50">
                Di sini, Anda dapat melihat detail pemesanan, melakukan pembayaran, membuat pengaduan, dan mengelola
                informasi kamar Anda melalui Sistem Informasi Rusunawa UNJ.
            </p>
        </div>

        {{-- Menampilkan pesan error validasi umum dari backend --}}
        ...
        <form wire:submit="login" class="flex flex-col gap-6 w-full">
            ...
            <div>
                ...
                <flux:input wire:model="contractCode" id="contractCodeInput" name="contractCode"
                    :label="__('ID Pemesanan')" type="text" required autofocus autocomplete="off"
                    placeholder="Masukkan ID Pemesanan Anda" aria-describedby="contractCodeHelper" />
                ...
                <div class="flex items-center gap-2" @paste="handlePaste">
                    @for ($i = 0; $i < 5; $i++)
                        <input type="text" maxlength="1" inputmode="numeric" pattern="[0-9]*"
                            ...
                            {{-- Menggunakan event listener yang paling dasar dan stabil --}} @input="handleInput($event)" @keydown="handleBackspace($event)">
                    @endfor
                </div>
