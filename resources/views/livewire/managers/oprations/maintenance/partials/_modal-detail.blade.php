            ...
            <x-managers.ui.card class="p-4">
                ...
                <div class="space-y-3">
                    @if ($recordType === \App\Enums\MaintenanceRecordType::ROUTINE->value)
                        ...
                        {{-- Removed: Terlambat --}}
                        {{-- Removed: <div class="flex justify-between items-center py-2">
                            <span class="text-gray-600 dark:text-gray-300">Terlambat</span>
                            @if($isLate)
                                <flux:icon.x-circle class="w-5 h-5 text-red-500 dark:text-red-400 inline-block" />
                            @else
                                <flux:icon.check-circle class="w-5 h-5 text-green-500 dark:text-green-400 inline-block" />
                            @endif
                        </div> --}}
                    @else {{-- For Urgent Maintenance --}}
                        <div class="flex justify-between items-center py-2 border-b border-gray-100 dark:border-zinc-700">
                        ...
                        @foreach ($existingRecordAttachments as $attachment)
                            <a href="{{ \Illuminate\Support\Facades\Storage::url($attachment['path']) }}" target="_blank" class="block">
                                @if (\Illuminate\Support\Str::startsWith($attachment['mime_type'], 'image'))
                                    <img src="{{ \Illuminate\Support\Facades\Storage::url($attachment['path']) }}" class="w-full h-24 object-cover rounded-lg shadow-sm border border-gray-200 dark:border-zinc-700" alt="{{ $attachment['name'] }}">
                                @else
                                    <div class="w-full h-24 flex flex-col items-center justify-center bg-gray-100 rounded-lg shadow-sm text-gray-500 dark:bg-zinc-700 dark:text-gray-400 border border-gray-200 dark:border-zinc-700">
