...
<html lang="id">
    ...
    <head>
        ...
        <title>Verifikasi Pembayaran Ditolak</title>
        <style>
            body {
                        ...
                        <tr>
                            <td style="padding: 30px 40px; color: #333333;">
                                <h1 style="font-size: 24px; color: #dc3545; margin-top: 0; font-weight: 600;">
                                    Mohon Maaf, Verifikasi Pembayaran Anda Ditolak
                                </h1>
                                <p style="font-size: 16px; line-height: 1.6; color: #4a5568;">
                                    Halo <strong>{{ $payment->invoice->contract->pic->full_name }}</strong>,
                                </p>
                                <p style="font-size: 16px; line-height: 1.6; color: #4a5568;">
                                    Verifikasi pembayaran Anda untuk Invoice #{{ $payment->invoice->invoice_number }}
                                    sebesar
                                    Rp{{ number_format($payment->amount_paid, 0, ',', '.') }} telah ditolak.
                                </p>
                                <p style="font-size: 16px; line-height: 1.6; color: #4a5568;">
                                    Mohon periksa kembali bukti pembayaran yang Anda unggah atau hubungi administrator
                                ...
                                <ul style="font-size: 16px; line-height: 1.6; color: #4a5568;">
                                    ...
                                    <li>Jumlah Pembayaran: Rp{{ number_format($payment->amount, 0, ',', '.') }}</li>
                                    <li>Jumlah yang harus dibayar: Rp{{ number_format($invoice->amount, 0, ',', '.') }}
                                    </li>
                                    <li>Tanggal Verifikasi: {{ $verificationLog->processed_at->format('d M Y H:i') }}
                                    </li>
                                    <li>Catatan dari Admin: {{ $verificationLog->reeason ?? 'Tidak ada catatan.' }}</li>
                                </ul>
